...
type IBGPSync struct {
	...
	// Guards the fire-and-forget sync triggered by UpdatePeersFromAPI so
	// overlapping calls don't stack concurrent BIRD reconfigures.
	syncing atomic.Bool
}
...
func (i *IBGPSync) UpdatePeersFromAPI(apiPeers []BirdIBGPPeer) {
	...
	// Trigger a sync immediately after receiving peers, but skip if one is
	// already running so overlapping updates don't stack concurrent BIRD
	// reconfigures (the periodic Run loop will pick up any missed change).
	if len(apiPeers) > 0 && i.syncing.CompareAndSwap(false, true) {
		go func() {
			defer i.syncing.Store(false)
...
func (i *IBGPSync) generateConfig(peer *MeshPeer, filename string) (bool, error) {
	...
	localASN := i.config.Node.ASN
	if localASN == 0 {
		localASN = config.DefaultLocalASN
	}
...
protocol bgp ibgp_{{.NodeID}} {
    local as {{.LocalASN}};
    neighbor {{.LoopbackIPv6}} as {{.LocalASN}};
